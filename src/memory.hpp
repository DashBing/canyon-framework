...
class Memory{
    ...
    std::vector<wordlength_t> memindex;

    public:
    void add_memory_block(wordlength_t size_byte){
    ...
    uint8_t & at(wordlength_t index){
        ...
        else if(memindex.size()>1)for(wordlength_t i=1; i<memindex.size(); i++){
